...
class _MyBottomDrawerState extends State<MyBottomDrawer> {
  ...
  Future searchData({required String todo}) async {
    final snapRef = await FirebaseFirestore.instance
        ...
        .get();
    final data = snapRef.data()!.entries.toList();
  ...
  Widget build(BuildContext context) {
    ...
    double headerSize = width * (isPortrait ? 0.05 : 0.03);
    double subTextSize = width * (isPortrait ? 0.04 : 0.025);
    var startDateField = ListTile(
      title: TextFormField(
      ...
      leading: Text('Start Date',
          style: TextStyle(fontSize: subTextSize, fontWeight: FontWeight.w600)),
      trailing: IconButton(
        icon: const Icon(Icons.calendar_today),
        onPressed: _startSelectDateTime,
      ),
    );
    var endDateField = ListTile(
      title: TextFormField(
    ...
    return Container(
      height: height * (isPortrait ? 0.4 : 0.5),
                  ...
                  children: [
                    Expanded(
                      ...
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.spaceAround,
                        crossAxisAlignment: CrossAxisAlignment.center,
                        children: [startDateField, endDateField],
                      ),
                    ),
                    Expanded(
                      flex: 1,
